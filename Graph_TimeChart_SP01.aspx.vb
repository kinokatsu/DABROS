...
Partial Class Graph_TimeChartSP01
    ...
    Private Structure GrpInfo
        ...
        Dim alertStyle As Integer               ' 管理値エリア塗りつぶし方法のスタイル　0:新しい方法（色指定）　1:以前のまま（色固定）
        Dim constInfName As String              ' 施工情報項目名
        Dim contourWidth As Integer             ' コンター図画像幅
        Dim contourHeight As Integer            ' コンター図画像高さ
        Dim Box1 As RectangleF                  ' 測点位置表示１
        Dim Box2 As RectangleF                  ' 測点位置表示２
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        ''Response.AddHeader("Expires", DateTime.Now.ToString)
        ...
        grpDataFile = Server.MapPath(siteDirectory + "\App_Data\TimeChartSP01GraphInfo.mdb")
        ...
        If IsPostBack = False Then
            ...
            Call GetMostUpdate()                                                '複数データファイルがある場合でも、最新日時を取得する
        ...
        strParm = clsCont.getConHeightInf(GrpProp.constInfName)
        Dim pointBoxRound As Single = Convert.ToSingle(Convert.ToSingle(strParm(14)) * 0.5)
        ...
        sb.Append(String.Format("<image id='svgimg' xlink:href='./api/thumbnail.ashx?img={0}'", strParm(5)))
        ...
        sb.Append(String.Format("<rect id='box1' x='0' y='0' width='0' height='0' rx='{0}' ry='{1}' class='PointBox'/>", pointBoxRound, pointBoxRound))
        sb.Append(String.Format("<rect id='box2' x='0' y='0' width='0' height='0' rx='{0}' ry='{1}' class='PointBox'/>", pointBoxRound, pointBoxRound))
        sb.Append(String.Format("<path id='pathline' d='' class='PointBox'/>"))
        sb.Append(String.Format("<text id='word' x='0' y='0' class='{0}' dominant-baseline='central'>{1}</text></svg>", strParm(11), strParm(6)))
        Me.litoutSvg.Text = sb.ToString                                                                  ' SVGの定義を記載
        sb.Length = 0

        '＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝コンター図関連
        clsCont.setDBPath = IO.Path.Combine(Server.MapPath(siteDirectory), "App_Data", "ContourPic.mdb")
        Dim showDate As DateTime
        If Me.RdBFromNewest.Checked = True Then
            showDate = DateTime.Now
        Else
            showDate = GrpProp.EndDate
        End If
        Dim contourFileas As String = clsCont.getFileNameFromDate(showDate)
        Dim imgUrl As String = String.Format("./api/thumbnail.ashx?img={0}&w={1}&h={2}", ("contour/" & contourFileas), GrpProp.contourWidth, GrpProp.contourHeight)
        Me.contour.ImageUrl = imgUrl
        If File.Exists(Server.MapPath(siteDirectory & "\img\contour\" & contourFileas)) = False Then
            Me.contour.ToolTip = "指定日時のコンター図は存在しません"
        Else
            Me.contour.ToolTip = showDate.ToString       'contourFileas
        End If
        'System.Diagnostics.Debug.WriteLine(contourFileas)
        ...
        Call SetControlVisible()                                                                'グラフ形式書式設定

        '' ビューステートへ変数を格納(コントロールに配置していないもののみ) ｺﾝﾄﾛｰﾙに配置しているものは後で読めるから
        Call SetViewState()

        Call SetGraphCommonInit(Convert.ToInt32(strParm(24)))
        ...
        sb.Capacity = 200
        sb.Append(strParm(7)).Append(",")
        sb.Append(strParm(8)).Append(",")
        sb.Append(strParm(16)).Append(",")
        sb.Append(strParm(15)).Append(",")
        sb.Append(strParm(17)).Append(",")
        sb.Append(strParm(13)).Append(",")
        sb.Append(GrpProp.Box1.X).Append(",")
        sb.Append(GrpProp.Box1.Y).Append(",")
        sb.Append(GrpProp.Box1.Width).Append(",")
        sb.Append(GrpProp.Box2.X).Append(",")
        sb.Append(GrpProp.Box2.Y).Append(",")
        sb.Append(GrpProp.Box2.Width)
        Me.conParam.Value = sb.ToString
        sb.Length = 0
        sb.Capacity = 16
        ...
        ''clsCont.setDBPath = Server.MapPath(siteDirectory) & "\App_Data\" & DataFileNames(0).CommonInf.ToString        '' SetGraphCommonInitに移設
    ...
    Public Sub SetBIND2Controls(ByVal dbCon As OleDb.OleDbConnection)
        ...
        ''■■スケール設定
        Dim Defval(3) As DropDownList
        Dim iloop As Integer

        'オブジェクトの指定
        Defval(0) = Me.DdlScale1 : Defval(2) = Me.DdlScaleR1
        Defval(1) = Me.DdlScale2 : Defval(3) = Me.DdlScaleR2
    ...
    Protected Sub GetMostUpdate(Optional ByVal DataFileNo As Integer = 0)
        ...
        For iloop = 0 To GrpProp.LineInfo.Length - 1                        '使用しているデータファイル番号の中で最新のデータ日時を取得する
            dteSession = CType(Session.Item("LastUpdate" & GrpProp.LineInfo(iloop).DataFileNo.ToString), DateTime)
        ...
        Try
            With DtSet.Tables("DData")
                If .Rows.Count > 0 Then
                    ...
                    GrpProp.constInfName = .Rows(0).Item(45).ToString                   ' 施工情報項目名 2017/03/14 Kino Add
                    GrpProp.contourWidth = Convert.ToInt32(.Rows(0).Item(46))           ' コンター図画像幅
                    GrpProp.contourHeight = Convert.ToInt32(.Rows(0).Item(47))          ' コンター図画像高さ
                    GrpProp.Box1.X = Convert.ToSingle(.Rows(0).Item(48))                ' 測点１ X座標
                    GrpProp.Box1.Y = Convert.ToSingle(.Rows(0).Item(49))                ' 測点１ Y座標
                    GrpProp.Box1.Width = Convert.ToSingle(.Rows(0).Item(50))            ' 測点１ 枠幅
                    GrpProp.Box2.X = Convert.ToSingle(.Rows(0).Item(51))                ' 測点２ X座標
                    GrpProp.Box2.Y = Convert.ToSingle(.Rows(0).Item(52))                ' 測点２ Y座標
                    GrpProp.Box2.Width = Convert.ToSingle(.Rows(0).Item(53))            ' 測点２ 枠幅
    ...
    Protected Function GraphLineInfoFromDB(ByVal dbCon As OleDb.OleDbConnection, Optional ByVal postBackFlg As Boolean = False) As Integer
        ...
        strSQL = (String.Format("Select * From 描画情報 WHERE 項目名='{0}' ORDER BY グラフNo ASC;", GrpProp.WindowTitle))
        DbDa = New OleDb.OleDbDataAdapter(strSQL, dbCon)
        DbDa.Fill(DtSet, "DData")

        Try
            For Each DTR As DataRow In DtSet.Tables("DData").Rows
                If DTR.ItemArray.GetValue(3).ToString.Length <> 0 Then
                    GrpProp.LineInfo(iloop).SubTitle = DTR.ItemArray.GetValue(3).ToString               'サブタイトル
                Else
                    GrpProp.LineInfo(iloop).SubTitle = ""                                               'サブタイトルなし
                End If
                With DTR.ItemArray
                    GrpProp.LineInfo(iloop).EnableRightAxis = Convert.ToBoolean(.GetValue(4))           '右軸の有効／無効
                    GrpProp.LineInfo(iloop).LeftAxis.DefaultSet = Convert.ToBoolean(.GetValue(5))       '左軸   既定値かどうか
                    GrpProp.LineInfo(iloop).LeftAxis.DefaultValue = Convert.ToInt32(.GetValue(6))       '       既定値の場合の値
                    GrpProp.LineInfo(iloop).LeftAxis.DataTitle = .GetValue(7).ToString                  '       データタイトル
                    GrpProp.LineInfo(iloop).LeftAxis.UnitTitle = .GetValue(8).ToString                  '       単位タイトル
                    GrpProp.LineInfo(iloop).LeftAxis.Max = Convert.ToSingle(.GetValue(9))               '       スケール最大値
                    GrpProp.LineInfo(iloop).LeftAxis.Min = Convert.ToSingle(.GetValue(10))              '       スケール最小値
                    GrpProp.LineInfo(iloop).LeftAxis.UnitMajor = Convert.ToSingle(.GetValue(11))        '       主メモリ間隔    
                    GrpProp.LineInfo(iloop).LeftAxis.UnitMinor = Convert.ToSingle(.GetValue(12))        '       副メモリ間隔
                    GrpProp.LineInfo(iloop).LeftAxis.DataCount = Convert.ToInt16(.GetValue(13))         '       データ系列数
                    GrpProp.LineInfo(iloop).DataFileNo = Convert.ToInt32(.GetValue(23))                 '       データファイル番号
                    GrpProp.LineInfo(iloop).LeftAxis.ChartType = .GetValue(24).ToString                 '       グラフ種別
                    GrpProp.LineInfo(iloop).LeftAxis.Reversed = Convert.ToBoolean(.GetValue(26))        '       軸反転
                    GrpProp.LineInfo(iloop).LeftAxis.LogAxis = CType(.GetValue(28), Boolean)        '       対数軸
                    GrpProp.LineInfo(iloop).spAlertValue = .GetValue(30).ToString                   '       個別警報値
                    If DtSet.Tables("DData").Columns.Count >= 31 Then                                   ' フィールド数チェック
                        GrpProp.LineInfo(iloop).locationX = CType(.GetValue(31), Integer)               '       チャートエリアの起点位置 X座標
                        GrpProp.LineInfo(iloop).locationY = CType(.GetValue(32), Integer)               '       チャートエリアの起点位置 Y座標
                        GrpProp.LineInfo(iloop).widthRatio = CType(.GetValue(33), Integer)              '       グラフ横幅に対するチャートエリアの割合
                        GrpProp.LineInfo(iloop).heightRatio = CType(.GetValue(34), Integer)             '       グラフ高さに対するチャートエリアの割合
                        GrpProp.LineInfo(iloop).subTitleSize = CType(.GetValue(35), Single)             '       サブタイトルのフォントサイズ
                        GrpProp.LineInfo(iloop).legendWidth = CType(.GetValue(36), Integer)             '       凡例の横幅
                        GrpProp.LineInfo(iloop).alertType = CType(.GetValue(37), Integer)               '       警報表示種別
                        GrpProp.LineInfo(iloop).showAlertValue = CType(.GetValue(38), Boolean)          '       警報値数値表示
                        GrpProp.LineInfo(iloop).alertValuePosition = CType(.GetValue(39), Integer)      '       警報値数値表示の場合の左から位置割合
                    Else
                    ...
                    '右軸が有効なら読み込む
                    If GrpProp.LineInfo(iloop).EnableRightAxis = True Then
                        GrpProp.LineInfo(iloop).RightAxis.DefaultSet = Convert.ToBoolean(.GetValue(14)) '右軸   既定値かどうか
                        GrpProp.LineInfo(iloop).RightAxis.DefaultValue = Convert.ToInt32(.GetValue(15)) '       既定値の場合の値
                        GrpProp.LineInfo(iloop).RightAxis.DataTitle = .GetValue(16).ToString            '       データタイトル
                        GrpProp.LineInfo(iloop).RightAxis.UnitTitle = .GetValue(17).ToString            '       単位タイトル
                        GrpProp.LineInfo(iloop).RightAxis.Max = Convert.ToSingle(.GetValue(18))         '       スケール最大値
                        GrpProp.LineInfo(iloop).RightAxis.Min = Convert.ToSingle(.GetValue(19))         '       スケール最小値
                        GrpProp.LineInfo(iloop).RightAxis.UnitMajor = Convert.ToSingle(.GetValue(20))   '       主メモリ間隔    
                        GrpProp.LineInfo(iloop).RightAxis.UnitMinor = Convert.ToSingle(.GetValue(21))   '       副メモリ間隔
                        GrpProp.LineInfo(iloop).RightAxis.DataCount = Convert.ToInt16(.GetValue(22))    '       データ系列数
                        GrpProp.LineInfo(iloop).RightAxis.ChartType = .GetValue(25).ToString            '       グラフ種別
                        GrpProp.LineInfo(iloop).RightAxis.Reversed = Convert.ToBoolean(.GetValue(27))   '       軸反転
                        GrpProp.LineInfo(iloop).RightAxis.LogAxis = Convert.ToBoolean(.GetValue(29))    '       対数軸
    ...
    Protected Function GraphDataInfoFromDB(ByVal dbCon As OleDb.OleDbConnection) As Integer
        ...
        strSQL = String.Format("Select * From 描画データ情報 WHERE 項目名='{0}' Order by グラフNo asc,データNo asc", GrpProp.WindowTitle)
        DbDa = New OleDb.OleDbDataAdapter(strSQL, dbCon)
        DbDa.Fill(DtSet, "DData")
        ''DbCom = New OleDb.OleDbCommand(strSQL, dbCon)
        ''DbDr = DbCom.ExecuteReader
        ...
        Try
            For Each DTR As DataRow In DtSet.Tables("DData").Rows                                                               'グラフ数でループ
                With DTR.ItemArray
                    GraphNo = Convert.ToInt32(.GetValue(2)) - 1
                    DataNo = Convert.ToInt32(.GetValue(3)) - 1
                    If GraphNo >= GrpProp.LineInfo.Length Then Exit For
                    GrpProp.LineInfo(GraphNo).LeftAxisData(DataNo).Symbol = .GetValue(4).ToString                               '計器記号
                    GrpProp.LineInfo(GraphNo).LeftAxisData(DataNo).ChNo = Convert.ToInt32(.GetValue(5))                         'チャンネル番号
                    GrpProp.LineInfo(GraphNo).LeftAxisData(DataNo).Color = Convert.ToInt32(.GetValue(6))                        '線色
                    GrpProp.LineInfo(GraphNo).LeftAxisData(DataNo).LineType = Convert.ToInt16(.GetValue(7))                     '線種
                    GrpProp.LineInfo(GraphNo).LeftAxisData(DataNo).LineWidth = Convert.ToInt16(.GetValue(8))                    '線幅
                    GrpProp.LineInfo(GraphNo).LeftAxisData(DataNo).Marker = Convert.ToInt16(.GetValue(9))                       'マーカ
                    ...
                    If GrpProp.LineInfo(GraphNo).EnableRightAxis = True And GrpProp.LineInfo(GraphNo).RightAxis.DataCount >= (DataNo + 1) Then
                        GrpProp.LineInfo(GraphNo).RightAxisData(DataNo).Symbol = .GetValue(10).ToString                         '計器記号
                        GrpProp.LineInfo(GraphNo).RightAxisData(DataNo).ChNo = Convert.ToInt32(.GetValue(11))                   'チャンネル番号
                        GrpProp.LineInfo(GraphNo).RightAxisData(DataNo).Color = Convert.ToInt32(.GetValue(12))                  '線色
                        GrpProp.LineInfo(GraphNo).RightAxisData(DataNo).LineType = Convert.ToInt16(.GetValue(13))               '線種
                        GrpProp.LineInfo(GraphNo).RightAxisData(DataNo).LineWidth = Convert.ToInt16(.GetValue(14))              '線幅
                        GrpProp.LineInfo(GraphNo).RightAxisData(DataNo).Marker = Convert.ToInt16(.GetValue(15))                 'マーカ
    ...
    Protected Sub SetControlVisible()
        ...
        Dim iloop As Integer
        Dim GrpBox(1) As WebControls.Panel
        Dim Grps(1) As C1WebChart
        ''Dim LabelNo(5) As WebControls.Label
        ''Dim SetPanel(5) As WebControls.Panel
        Dim SetPanelR(1) As WebControls.Panel
        Dim GrpHeight As Integer
        Dim GrpWidth As Integer
        Dim ChartHeight As Integer
        Dim ChartWidth As Integer
        Dim RExValMax(2) As RegularExpressionValidator      '2009/11/19
        Dim RExValMaxR(2) As RegularExpressionValidator
        Dim RExValMin(2) As RegularExpressionValidator
        Dim RExValMinR(2) As RegularExpressionValidator
        Dim ValidCOExMax(2) As AjaxControlToolkit.ValidatorCalloutExtender
        Dim ValidCOExMin(2) As AjaxControlToolkit.ValidatorCalloutExtender
        Dim tableRow(1) As HtmlControls.HtmlTableRow
        ...
        For iloop = 0 To 1
            ...
            If iloop <= GrpProp.GraphCount - 1 Then
                ...
                End If
            Else
                ''LabelNo(iloop).Visible = True                                   '' 2012/11/21 Kino Add 表の枠線だけがのこるので、No.ラベルだけは残すようにした 
                ''LabelNo(iloop).ForeColor = Color.Gray                             '' 2015/11/25 Kino Changed jQueryによるDOMで行を削除するように変更
                Me.Controls.Remove(Grps(iloop))
                Grps(iloop).Dispose()

                ''Me.Controls.Remove(LabelNo(iloop))
                ''LabelNo(iloop).Dispose()
                ''Me.Controls.Remove(SetPanel(iloop))
                ''SetPanel(iloop).Dispose()
                ''Me.Controls.Remove(SetPanelR(iloop))
                ''SetPanelR(iloop).Dispose()

                SetPanelR(iloop).Visible = False

                Me.Controls.Remove(RExValMax(iloop))           '2009/11/19
                RExValMax(iloop).Dispose()
                Me.Controls.Remove(RExValMin(iloop))
                RExValMin(iloop).Dispose()
                Me.Controls.Remove(RExValMaxR(iloop))
                RExValMaxR(iloop).Dispose()
                Me.Controls.Remove(RExValMinR(iloop))
                RExValMinR(iloop).Dispose()
                Me.Controls.Remove(ValidCOExMax(iloop))
                ValidCOExMax(iloop).Dispose()
                Me.Controls.Remove(ValidCOExMin(iloop))
                ValidCOExMin(iloop).Dispose()
        ...
        For iloop = 0 To GrpProp.GraphBoxCount - 1
            With Grps(iloop)
                ...
                Select Case GrpProp.PaperOrientaion
                    Case 0      'A4縦の場合
                        GrpHeight = Convert.ToInt32(Fix(460 / GrpProp.GraphBoxCount))            '913
                        GrpWidth = 900
                        ChartHeight = GrpHeight
                        If GrpProp.EnableLegend = 1 Then
                            ChartWidth = 900
                        Else
                            ChartWidth = 870 'ChartWidth = 520 Else ChartWidth = 620
                        End If
                        ''Me.GraphPage.CssClass = Nothing
                        'Me.GraphPage.Attributes.Clear()
                        ''Me.GraphPage.CssClass = "GraphOuterPortrait"
                        'Me.GraphPage.Attributes.Add("class", "GraphOuterPortrait")

                    Case 1      'A4横の場合
                        GrpHeight = Convert.ToInt32(Fix(340 / GrpProp.GraphBoxCount))       '350
                        GrpWidth = 990
                        ChartHeight = GrpHeight
                        ChartWidth = 990
                        ''Me.GraphPage.CssClass = Nothing
                        'Me.GraphPage.Attributes.Clear()
                        ''Me.GraphPage.CssClass = "GraphOuterLandscape"
                        'Me.GraphPage.Attributes.Add("class", "GraphOuterLandscape")
        ...
        Me.LblTitleUpper.Text = GrpProp.LineInfo(0).SubTitle
        Me.LblTitleUpper.Font.Size = GrpProp.DrawSizeColorInf(0)
        'Me.LblTitleUpper.Width = GrpWidth
        Me.TitleUpperOut.Style("width") = (GrpWidth & "px")

        If GrpProp.GraphCount = 2 Then
            Me.LblTitleLower.Text = GrpProp.LineInfo(1).SubTitle
            Me.LblTitleLower.Font.Size = GrpProp.DrawSizeColorInf(0)
            'Me.LblTitleLower.Width = GrpWidth
            Me.TitleLowerOut.Style("width") = (GrpWidth & "px")
        Else
            Me.LblTitleLower.Visible = False
            Me.TitleLowerOut.Visible = False
        End If
        ...
        For iloop = 1 To 2                                              ' 2015/11/25 Kino Add
    ...
    ''' 経時変化図描画メインプロシージャ
    ''' </summary>
    ''' <param name="conHeightCh">コンクリート打設位置保存チャンネル</param>
    ''' <remarks></remarks>
    Protected Sub SetGraphCommonInit(conHeightCh As Integer)
        ...
        For iloop = 0 To GrpProp.GraphCount - 1
            ...
            With Grps(iloop)
                ...
                .Legend.Style.Font = New Font("Meiryo", GrpProp.DrawSizeColorInf(16))       ' フィールドインデックス-13

                '●●●●●　X1軸設定
                ''Call SetAxisYProperties(Grps(iloop), 0, iloop, Defval)
                With .ChartArea.AxisX
                    ''.Visible = True                     
                    ''.Font = New Font("MS UI Gothic", GrpProp.DrawSizeColorInf(8))       ' フィールドインデックス-13
                    .Font = New Font("ＭＳ Ｐゴシック", GrpProp.DrawSizeColorInf(8))       ' フィールドインデックス-13
                    ...
                    .GridMajor.Color = Color.Silver
                    .GridMinor.Pattern = LinePatternEnum.Dot
            ...
            For iloop = 0 To GrpProp.GraphCount - 1
                With Grps(iloop)
                    ...
                    If UsePlotArea(iloop) = True Then
                        ...
                        'mapData.Tooltip = "{#XVAL:0.0} [{#YVAL:0.0}]"                     'データの表示
                        'mapData.Tooltip = "({#XVAL} [{#YVAL}])"                               'データの表示
                        mapData.Tooltip = "測定日時　：{#XVAL:yyyy/MM/dd HH:mm}<br />数値データ：{#YVAL:0.00}"                               'データの表示
                        'mapData.Tooltip = "測定日時：{#XVAL:yyyy/MM/dd HH:mm}　Y軸データ：{#YVAL:0.00}"                               'データの表示
        ...
        '================================================ [ コンクリート打設位置表示情報 ] ================================================↑
        Dim clsCont As New ClsContour

        If RdBFromNewest.Checked = True Then            ' 最新データ表示の場合はCommonInfから読み込む
            clsCont.setDBPath = Server.MapPath(siteDirectory) & "\App_Data\" & DataFileNames(0).CommonInf.ToString
            Me.conheight.Value = clsCont.ReadDataSummary4Yanagawa(conHeightCh, 19)

        Else                                            ' 過去データ表示の場合　（未確認）
            AccDataFile = Server.MapPath(siteDirectory & "\App_Data\" & DataFileNames(0).FileName)
            ReDim DataCh(0)
            DataCh(0) = conHeightCh
            'intRet = clsNewDate.GetDrawData(AccDataFile, Convert.ToInt16(RLDataCount), DataCh, StDate, EdDate, Datas, "")          ’2017/09/11 Kino Changed　to Lower Statement
            intRet = clsNewDate.GetDrawData(AccDataFile, Convert.ToInt16(0), DataCh, StDate, EdDate, Datas, "")                     ' コンクリート打設高さは1チャンネルしかない
            Me.conheight.Value = Datas(Datas.Length - 1).GetValue(1).ToString
        End If
    ...
    Protected Sub SetAxisYProperties(ByVal tWC As C1WebChart, ByVal leftRightFlg As Integer, ByVal loopCount As Integer, ByVal defval() As DropDownList) ', _
        ...
        With tWC
            With SetAxis
                .Font = New Font("ＭＳ Ｐゴシック", GrpProp.DrawSizeColorInf(8))
                .Visible = True
                .ForeColor = Drawing.Color.Black                                '線色
                ''.Thickness = PlotAreBorder
                .TickMinor = TickMarksEnum.None                              '副メモリの突出方向
    ...
    Protected Sub SetViewState()
        ...
        ViewState("constInf") = GrpProp.constInfName        ' 施工情報項目名
        ViewState("conWid") = GrpProp.contourWidth          ' コンター図画像幅
        ViewState("conHei") = GrpProp.contourHeight         ' コンター図画像高さ

        ViewState("b1x") = GrpProp.Box1.X
        ViewState("b1y") = GrpProp.Box1.Y
        ViewState("b1w") = GrpProp.Box1.Width
        ViewState("b2x") = GrpProp.Box2.X
        ViewState("b2y") = GrpProp.Box2.Y
        ViewState("b2w") = GrpProp.Box2.Width
    ...
    Protected Sub ReadViewState()
        ...
        GrpProp.constInfName = CType(ViewState("constInf"), String)                                                         ' 施工情報項目名
        GrpProp.contourWidth = Convert.ToInt32(ViewState("conWid"))                                                         ' コンター図画像幅
        GrpProp.contourHeight = Convert.ToInt32(ViewState("conHei"))                                                        ' コンター図画像高さ

        GrpProp.Box1.X = Convert.ToSingle(ViewState("b1x"))
        GrpProp.Box1.Y = Convert.ToSingle(ViewState("b1y"))
        GrpProp.Box1.Width = Convert.ToSingle(ViewState("b1w"))
        GrpProp.Box2.X = Convert.ToSingle(ViewState("b2x"))
        GrpProp.Box2.Y = Convert.ToSingle(ViewState("b2y"))
        GrpProp.Box2.Width = Convert.ToSingle(ViewState("b2w"))
    ...
    Protected Sub Set2FormFormControl()
        ...
        Dim iloop As Integer
        Dim ScaleMax(1), ScaleMaxR(1) As WebControls.TextBox
        Dim ScaleMin(1), ScaleMinR(1) As WebControls.TextBox
        Dim ScaleDef(1), ScaleDefR(1) As WebControls.RadioButton
        Dim ScaleInput(1), ScaleInputR(1) As WebControls.RadioButton
        Dim DefVal(1), DefValR(1) As DropDownList
    ...
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Protected Sub lkBtn_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lkBtn1.Click, lkBtn2.Click
        ...
        Dim grpDataFile As String = Server.MapPath(siteDirectory + "\App_Data\TimeChartSP01GraphInfo.mdb")
    ...
    Private Sub saveScaleSet(ByVal graphName As String, ByVal btnNo As Integer, ByVal grpDataFile As String, Optional ByVal leftRightFlg As Integer = 0)
        ...
        dataAdp = New OleDbDataAdapter("SELECT * FROM 描画情報 WHERE グラフNo=" & btnNo.ToString & " AND 項目名='" & graphName & "'", cn)
        dataAdp.Fill(dSet, "GraphName")
        DTable = dSet.Tables("GraphName")

        ' 主キーの列数を取得
        cn.Open()
        schemaTable = cn.GetOleDbSchemaTable(OleDbSchemaGuid.Primary_Keys, New Object() {Nothing, Nothing, "描画情報"})      'graphName})
        cn.Close()

        If schemaTable.Rows.Count = 0 Then                                                                  ' 主キーがあると、カウンター値が入るらしい。
            cn.Open()
            Dim strSQL As String = "ALTER TABLE 描画情報 ALTER COLUMN ID LONG NOT NULL"         '[" & graphName & "]
            com.Connection = cn
            com.CommandText = strSQL
            com.ExecuteNonQuery()
            strSQL = "ALTER TABLE 描画情報 ADD CONSTRAINT IDNum PRIMARY KEY (ID)"                '[" & graphName & "]
            com.CommandText = strSQL
            com.ExecuteNonQuery()
